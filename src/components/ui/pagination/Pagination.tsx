...
export const Pagination = ({ totalPages }: Props) => {
  ...
  const createPageUrl = (pageNumber: number | string) => {
    ...
    // se queda en la pagina que se encuentra al apretar en los "..."
    if (pageNumber === "...") {
      return `${pathname}?${params.toString()}`
    }

    if (+pageNumber <= 0) {
      return `${pathname}` // href="/", href="/kid"
    }

    // no hya pagina 6 por ejemplo entonces se queda en la que se encuentra , la 5
